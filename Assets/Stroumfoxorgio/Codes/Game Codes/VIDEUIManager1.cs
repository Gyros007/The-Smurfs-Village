...
public class VIDEUIManager1 : MonoBehaviour
{
        ...
        if (VD.isActive) //If there is a dialogue active
        {
            ...
            if (!data.pausedAction && data.isPlayer)
            {
                ...
                for (int i = 0; i < currentChoices.Count; i++)
                {
                    ...
                    currentChoices[i].fontSize = 12;

                    if (i == data.commentIndex)
                    {
                        currentChoices[i].color = new Color32(136, 204, 255, 255);
                         currentChoices[i].fontSize = 14;
                    }
                }
            }
        }
    ...
    bool PreConditions(VIDE_Assign dialogue)
    {
        ...
        } else //Stuff we do right before the dialogue begins
        {
            ...
            {
                case "Xefteris":
                    if (var.numXila == 3 && !var.returnedToXefteri && var.metXefteris)
                    {
                        dialogue.overrideStartNode = 4;
                        var.returnedToXefteri = true;
                    }
                    if (!var.metXefteris)
                    {
                        var.questPostitionXefteris = var.questCounter;
                        var.questCounter++;
                    }
                    var.metXefteris = true;
                    break;
                ...
                case "Wood":
                    ...
                    break;
                case "Stroumfita":
                    if (var.foundVase && var.foundFlower && !var.returnedToStroumfita && var.metStrοumfita)
                    {
                        dialogue.overrideStartNode = 6;
                        var.returnedToStroumfita = true;
                    }
                    if (!var.metStrοumfita)
                    {
                        var.questPostitionStoumfita = var.questCounter;
                        var.questCounter++;
                    }
                    var.metStrοumfita = true;
                    break;

                case "Flower":
                    var.foundFlower = true;
                    break;
                case "Vase":
                    var.foundVase = true;
                    break;
                case "PainterSmurf":
                    if (var.foundPaintBrush && !var.returnedToPainter && var.metPainter)
                    {
                        dialogue.overrideStartNode = 4;
                        var.returnedToPainter = true;
                    }
                    if (!var.metPainter){
                            var.questPostitionPainter = var.questCounter;
                            var.questCounter++;
                    }
                    var.metPainter = true;
                    break;
                ...
                case "PaintBrush":
                    var.foundPaintBrush = true;
                    break;
                    
                case "Gkriniaris":
                    if(var.foundSoap && !var.returnedToGkriniaris && var.metGkriniaris)
                	{
                	    dialogue.overrideStartNode= 1 ;
                	    var.returnedToGkriniaris=true;
                	}
                    if (!var.metGkriniaris)
                    {
                        if (!var.metPapastroumf)
                        {
                            var.questPostitionGkriniaris = var.questCounter;
                            var.questCounter++;
                        }
                    }
                    var.metGkriniaris = true;
                    break;
                ...
                case "Papastroumf":
                    ...
                    if (var.returnedToGkriniaris)
                    {
                        dialogue.overrideStartNode = 4;
                    }
                    var.metPapastroumf = true;
                    break;

                case "Xaxanoulis":
                    if(var.openPresent && !var.returnedToXaxanoulis)
                	{
                	dialogue.overrideStartNode= 1 ;
                	var.returnedToXaxanoulis=true;
                	}
                	else if (var.openPresent && var.returnedToXaxanoulis && var.metXaxanoulis)
                	{
                    ...
                    //if (!var.metXaxanoulis)
                    //{
                        //var.questPostitionXaxanoulis = var.questCounter;
                        //var.questCounter++;
                    //}
                    var.metXaxanoulis = true;
                    break;
                ...
                case "Lixoudis":
                    if (var.numApples == 3 && !var.returnedToLixoudis && var.metLixoudis)
                    {
                        dialogue.overrideStartNode = 6;
                        var.returnedToLixoudis = true;
                    }
                    if (!var.metLixoudis)
                    {
                        var.questPostitionLixoudis = var.questCounter;
                        var.questCounter++;
                    }
                    var.metLixoudis = true;
                    break;
